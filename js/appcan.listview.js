    ...
    l.prototype = {
        buildListview: function(b, c) {
            ...
            for (var h in b) {
                ...
                (e.option.hasCheckbox || e.option.hasRadiobox) && !
                function(a) {
                    a.find("input").on("change",
                    function(b) {
                        a[0].lv_data.checked = b.currentTarget.checked,
                        e.option.hasCheckbox && e.emit("checkbox:change", e, a, a[0].lv_data),
                        e.option.hasRadiobox && e.emit("radio:change", e, a, a[0].lv_data)
                    })
                } (j),
                d.append(j)
            }
            var l = a(".switch", d);
            return appcan.switchBtn(l,
            function(a, b) {
                var c = a.parent();
                c[0].lv_data.switchBtn.value = b,
                e.emit("switch:change", e, c, c[0].lv_data)
            }),
            d
        },
        buildGroupview: function(b) {
            ...
            for (var e in b) {
                ...
                d.append(f),
                d.append(c.buildListview(b[e].items, e))
            }
            return d
        },
        add: function(a, b) {
        ...
        set: function(a) {
            var b = this,
            c;
            return c = b.option.hasGroup ? b.buildGroupview(a) : b.buildListview(a),
            b.ele.html(c),
            b
        },
        itemClick: function(b) {
            ...
            if (d.removeClass(this.option.touchClass), c.option.hasCheckbox) {
                if (a(b.target).is("input[type=checkbox]")) return;
                var e = d.find("input");
                e[0].checked = !e[0].checked
            }
            if (c.option.hasRadiobox) {
                if (a(b.target).is("input[type=radio]")) return;
                var f = d.find("input");
                f[0].checked = !0
            }
        ...
        touchItem: function(b) {
            if (this.option.hasTouchEffect) {
                var c = this,
                d = a(b.currentTarget);
                if (d[0].lv_data.switchBtn && c.option.hasControl) return;
